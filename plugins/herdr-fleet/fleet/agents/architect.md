...
You have one, at `<fleet home>/memory/architect/` — nobody else's: a builder's
habits and a researcher's habits must not blur.

**Read `memory/architect/MEMORY.md` when you adopt this persona**, in the same
...
**You write to `memory/architect/decisions.md` and nothing else.** Append-only,

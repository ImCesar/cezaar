...
You have one, at `<fleet home>/memory/builder/` — nobody else's: a builder's
habits and a researcher's habits must not blur.

**Read `memory/builder/MEMORY.md` when you adopt this persona**, in the same
...
**You write to `memory/builder/decisions.md` and nothing else.** Append-only,
